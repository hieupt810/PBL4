...
export default function HomeInformation() {
  ...
  const handleAdd = async (images: File[] | null) => {
    ...
    try {
      const response = await fetch(
        `${process.env.BACKEND_URL}api/door/upload`,
        {
          method: "POST",
          headers: {
    ...
    fetch(process.env.BACKEND_URL + "api/user", {
      ...
      .then((d) => {
        if (d.code == 200) {
          ...
          setCookie("home_id", d.data.home_id);
    ...
    <div>
      ...
      <MobileLayout>
        ...
        <div className="fixed bottom-7 right-7">
          <ModalComponent
            icon={<LuImagePlus size={20} className="text--500" />}
            name="Add"
            title="Add image"
