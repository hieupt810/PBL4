...
def createIR():
    ...
    try:
        if (not "Authorization" in request.headers) or (
            not validRequest(request, requires)
        ):
            return respondWithError(msg="E002", code=400)

        records, _, _ = db.execute_query(
            query(
                """MATCH (u:User {token: $token, role: $role}) RETURN u.role AS role LIMIT 1"""
            ),
            routing_="r",
...
def addFunc():
    ...
    try:
        ...
        records, _, _ = db.execute_query(
            query("""CREATE (f:Func {mode:$mode, id:$id })"""),
            routing_="w",
            id=uniqueID(),
            mode=req["mode"],
        )
...
def getFuncs():
    try:
        records, _, _ = db.execute_query(
            query("""MATCH (func:Func) RETURN func"""),
            routing_="r",
        )
...
def addIr():
    ...
    try:
        ...
        records, _, _ = db.execute_query(
            ...
            ir_code=req[
                "ir_code"
            ],  # Thay YOUR_IR_CODE_HERE bằng giá trị thực của ir_code
        )
        return respond()
    except Exception as error:
        return respondWithError(code=500, error=error)
